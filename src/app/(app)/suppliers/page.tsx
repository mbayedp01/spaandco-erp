...
const categoryColor: Record<string, string> = {
  ...
  'Soins corps & Accessoires': 'bg-emerald-50 text-emerald-700',
...
export default function SuppliersPage() {
  ...
  const pending = suppliers.reduce((sum, s) => sum + s.pendingOrders, 0)
  const totalMonthly = suppliers.filter((s) => s.status === 'actif').reduce((sum, s) => sum + s.monthlySend, 0)
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {pending > 0 && (
          <div className="mb-5 flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
            ...
            <span><strong>{pending} commande{pending > 1 ? 's' : ''} en attente</strong> de livraison chez {suppliers.filter(s => s.pendingOrders > 0).map(s => s.name).join(', ')}</span>
          </div>
        )}
        ...
        <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="flex items-center gap-3 rounded-lg border border-stone-200 bg-white p-5 shadow-xs">
            ...
            <div>
              <p className="text-xs text-stone-500">Fournisseurs</p>
              <p className="text-2xl font-bold text-slate-900">{actifs.length}</p>
              <p className="text-[10px] text-stone-400">actifs</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border border-stone-200 bg-white p-5 shadow-xs">
                ...
                {suppliers.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-stone-50">
                      ...
                      <td className="hidden px-5 py-3.5 text-stone-500 lg:table-cell">{s.lastOrder}</td>
                      <td className="px-5 py-3.5 font-semibold text-slate-900">
                        {s.monthlySend.toLocaleString('fr-FR')} F
                      </td>
                      <td className="px-5 py-3.5">
