...
export default function ClientsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'nouveaux') list = list.filter((c) => new Date(c.joinDate) >= new Date('2026-05-01'))
    if (search.trim()) {
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-stone-400">{c.visitsCount} visite{c.visitsCount > 1 ? 's' : ''}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between text-xs">
                          <span className={cn('font-semibold', c.loyaltyPoints >= VIP_THRESHOLD ? 'text-amber-600' : 'text-primary-600')}>
                            {c.loyaltyPoints} pts
                          </span>
                          {c.isVip && (
                            <span className="rounded-full bg-amber-50 px-1.5 py-0.5 text-[10px] font-medium text-amber-700">VIP</span>
                          )}
                        </div>
                        <div className="h-1.5 w-24 overflow-hidden rounded-full bg-stone-100">
                          <div
                            className={cn('h-full rounded-full', c.loyaltyPoints >= VIP_THRESHOLD ? 'bg-amber-400' : 'bg-primary-500')}
                            style={{ width: `${Math.min((c.loyaltyPoints / LOYALTY_MAX) * 100, 100)}%` }}
                          />
                        </div>
                      </div>
                    </td>
